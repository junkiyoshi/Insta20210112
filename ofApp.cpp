...
void ofApp::update() {
	...
	for (int x = 220; x < ofGetWidth(); x += 280) {
		...
		for (int k = 0; k < 3; k++) {
			...
			for (int i = 0; i < 50; i++) {
				...
				auto noise_radius = ofMap(ofNoise(noise_seed_radius, (ofGetFrameNum() + i) * 0.004), 0, 1, -250, 250);
			...
			for (int deg = 0; deg < 360; deg += 60) {
				...
				for (int i = 0; i < vertices.size(); i++) {
					...
					line.addVertex(glm::vec4(vertices[i], 0) * rotation + glm::vec4(x, ofGetHeight() * 0.5, 0, 0));
					if (i > 0) {
		...
		for (int k = i + 1; k < this->line_list.size(); k++) {
			...
			for (int m = 0; m < this->line_list[i].getNumVertices(); m++) {
				...
				for (int p = 0; p < this->line_list[k].getNumVertices(); p++) {
					...
					if (glm::distance(this->line_list[i].getVertex(m), this->line_list[k].getVertex(p)) < 15) {
